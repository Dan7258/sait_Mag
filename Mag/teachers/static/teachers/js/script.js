...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    async function createCardList() {
        const data = divideData(await getData("/teacher/data"), elemsOnPage);
        data.forEach(i => {
        ...
        if (activeId != "") {
            insertOverlayCard(cards[(activeId - 1) % elemsOnPage], activeId - 1);
        }
        openCard(cards);
    }
    ...
    function insertOverlayCard(cardItem,index) {
        const card = {
            ...
            id: (index + 1) + ((activePage - 1) * elemsOnPage)
        };
        let link = window.location.href.match(/teacher\/.*/)[0].substring(7);
        if (!link.match(/id=\d+/)) {
            history.pushState(card.id, null, `/teacher${link}/?id=${card.id}`);
        }
        ...
        peopleOverlay.addEventListener("click", (e) => {
            ...
            if (crossBtn == e.target) {
                history.pushState(null, null, `/teacher/?page=${activePage}`);
    ...
    createCardList();
    createPagination(Math.ceil(data.length / elemsOnPage), 2);
    history.pushState(null, null, `/teacher/?page=${activePage}`);
});
